...
class STM_LMS_User_Manager_Course {
	...
	public function map_students( $student_course ) {
		...
		global $wpdb;
		$req = "SELECT user_id, meta_value
		FROM `wplms_usermeta`
		where user_id = {$user_id} and meta_key = 'vawsybutk1'";
		$data = STM_LMS_Helpers::simplify_db_array($wpdb->get_results($req, ARRAY_A));
		$student_course['region'] = $data['meta_value'];
